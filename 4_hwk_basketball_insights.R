...
#Insights function 
plotInsights <- function(data, rows=1:10){ #with default rows
  Data<-data[rows,,drop=F] 
  matplot(t(Data), type="b", pch=15:18, col=c(1:4,6))  #transpose matrix bc matplot always plots columns
  legend("bottomleft", inset=0.01, legend=Players[rows], col=c(1:4,6), pch=15:18) 
}

#FreeThrowAttempts per game
plotInsights(FreeThrowAttempts/Games)
#Freethrow accuracy
plotInsights(FreeThrows/FreeThrowAttempts)
#player playing style
plotInsights((Points-FreeThrows)/FieldGoals)

#Dwight Howard seems to aim for 2 pointers rather than 3 pointers
# that explains the low accuracy of free throws and the playing style which shows the line hovering around 2 points 